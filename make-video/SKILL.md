...
5. **QA the output:** `bun run qa <slug>` validates `final.mp4` (duration, 1080p, audio not silent, caption-cue count) and exits nonzero if any check fails. It also prints **advisory, non-blocking** OCR-based secret-leak warnings (review them — OCR can be wrong); `--no-secrets` skips that pass.
